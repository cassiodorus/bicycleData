...
if ( length(temp) > 0 ) {
	print( paste("There are", temp, "instances of BikeAge_Gr = '10-Jun' or '15-Nov'. Set these to NA") )
	rawDataFixedMiss <- rawDataFixedMiss %>% dplyr::mutate( BikeAge_Gr=ifelse(BikeAge_Gr %in% c("10-Jun","15-Nov"),NA,BikeAge_Gr) )
}
```

## Step 11. Noted by observation:   
...
## Step 12. Recompute ancillary data frames and check on changes  
```{r step12, error=FALSE}
 ...
 temp3 <- nrow( subset(rawDataFixedMiss_catcount, variable=="BikeAge_Gr" & value %in% c("10-Jun","15-Nov")) )
 temp4 <- nrow( subset(rawDataFixedMiss_catcount, variable=="Crash_Loc" &value=="Location") )
 temp5 <- nrow( subset(rawDataFixedMiss_catcount, variable=="Workzone_I" & value=="No0") )
